  ...
  const renderStars = (ratingCount) => {
    return Array.from({ length: 5 }, (_, index) => (
      <FaStar
        key={index}
        className={`h-4 w-4 ${
          index < ratingCount ? "text-green-500" : "text-gray-400"
        }`}
      />
    ));
  };
        ...
        <div>
          <div
            className={`font-semibold text-lg mb-1 ${
              title.length > 10 ? "text-sm md:text-lg mb-3" : "text-lg"
            }`}
          >
            {title}
          </div>
          <div className="flex">{renderStars(ratingCount)}</div>
        </div>
        {/* <div className="text-gray-800">{price}</div> */}
      </div>
      <img
        className="mx-auto w-3/4 object-contain -mt-14 md:-mt-20 lg:-mt-24"
        src={imageSrc}
        alt={title}
      />
    </div>
